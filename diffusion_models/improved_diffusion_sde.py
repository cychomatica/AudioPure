...
import torchsde
from .Improved_Diffusion_Unconditional.improved_diffusion.sc09_spectrogram_dataset import melspec_inv_standardize, melspec_standardize

from .Improved_Diffusion_Unconditional.improved_diffusion.script_util import create_model_and_diffusion, model_and_diffusion_defaults
from .Improved_Diffusion_Unconditional.improved_diffusion.sc09_spectrogram_dataset import *
...
class RevVPSDE(torch.nn.Module):
    def __init__(self, model, score_type='guided_diffusion', beta_min=0.1, beta_max=20, N=1000,
                 img_shape=(1, 32, 32), model_kwargs=None):
...
class RevImprovedDiffusion(torch.nn.Module):
    
    def __init__(self, args, config=None, device=None):
        super().__init__()
        self.args = args
        self.config = config
        if device is None:
        ...
        # load model
        img_shape = (1, 32, 32)
        model_dir = '_checkpoints'
        model_config = model_and_diffusion_defaults()
        # model_config.update(vars(self.config.model))
        # print(f'model_config: {model_config}')
        model, diffusion = create_model_and_diffusion(**model_config)
        model.load_state_dict(torch.load(args.ddpm_path, map_location='cpu'))
    ...
    def image_editing_sample(self, img):
        ...
        for it in range(self.args.sample_step):
            ...
            x_ = x.contiguous().view(batch_size, -1)  # (batch_size, state_size)
            if self.args.use_bm:
    ...
    def forward(self, x):
        ...
        output = self.image_editing_sample(x)
